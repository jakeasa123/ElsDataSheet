...
upgrade_rate_array = [
    ...
    [
        ...
        [0, 1000, 1000, 1000, 1000, 1000],
        [0, 400, 400, 1000, 1000, 1000],
        [0, 100, 100, 200, 500, 1000],
        [0, 40, 240, 320, 600, 1000],
        [0, 20, 270, 330, 600, 1000],

        [0, 14, 367, 430, 700, 1000],
        [0, 20, 270, 330, 600, 1000],
        [0, 20, 310, 330, 600, 1000],
    ]
]
...
function updateInfo(current_upgrade_level, input_data) {
    ...
    info_id_list.forEach(function(key) {
        document.getElementById(key).innerText = numberWithCommas(input_data[key])
    });
}
...
function normalUpgrade() {
    ...
    var input_dict = {}
    info_id_list.forEach(function(key) {
        input_dict[key] = parseInt(document.getElementById(key).innerText.replace(',', ''))
    });
    var use_el_hammer = document.getElementById('use-el-hammer').checked
    if (use_el_hammer) { use_el_hammer = 1 } else { use_el_hammer = 0 }

    // 強化次數固定 + 1
    input_dict['upgrade-time'] += 1
    if (use_el_hammer == 1) {
        input_dict['el-hammer'] += 1
    }
    ...
    // 判斷強化結果
    if (between(random_num, upgrade_rate_array[use_el_hammer][current_level][0], upgrade_rate_array[use_el_hammer][current_level][1])) {
        // 成功
    ...
    else if (between(random_num, upgrade_rate_array[use_el_hammer][current_level][1], upgrade_rate_array[use_el_hammer][current_level][2])) {
        // 破壞
    ...
    else if (between(random_num, upgrade_rate_array[use_el_hammer][current_level][2], upgrade_rate_array[use_el_hammer][current_level][3])) {
        // 無變化
        upgrade_note.value = current_time + ' [' + random_num.toString().padStart(4, '0') + '] 沒有任何變化……\n' + upgrade_note.value
        input_dict['no-change-time'] += 1
    }
    else if (between(random_num, upgrade_rate_array[use_el_hammer][current_level][3], upgrade_rate_array[use_el_hammer][current_level][4])) {
        // 下降
    ...
    else if (between(random_num, upgrade_rate_array[use_el_hammer][current_level][4], upgrade_rate_array[use_el_hammer][current_level][5])) {
        // 歸零
...
function protectUpgrade() {
    ...
    // 未達 +7，無法使用守護石
    if (current_level <= 6) {
        upgrade_note.value = current_time + ' 強化等級未達 +7，無法使用守護石。\n' + upgrade_note.value
        return
    }
    ...
    if (between(current_level, 7, 11)) {
        ...
        else if (between(random_num, upgrade_rate_array[use_el_hammer][current_level][2], upgrade_rate_array[use_el_hammer][current_level][5])) {
            // 無變化
            upgrade_note.value = current_time + ' [' + random_num.toString().padStart(4, '0') + '] 沒有任何變化……\n' + upgrade_note.value
            input_dict['no-change-time'] += 1
        }
